...
var Apps = {    
    ...
    Edge: {    
        ...
        init: () => {
            var element = Apps.Edge.element();
            TaskManager.init(Apps.Edge, element);
            DesktopManager.getWindowDimensions(Apps.Edge, 800, 800);
            AppManager.addWindowDrag(Apps.Edge.options.id);
            AppManager.addResizability(Apps.Edge.options.id);
        },

        element: () => {
            var view = `
                <iframe style="height: 100%; width: 100%" frameborder="none" src="http://bing.com"></iframe>
            `;
            return AppWindow.init(view, Apps.Edge.options)
        }
    },
}